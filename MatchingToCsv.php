    ...
    foreach ($masterArray as $i => $values) {
        ...
        foreach ($values as $itm => $value)
            {     
                 $header = "";
                if($first==true){
                    $currentMatchTo = $value;
                    fputcsv($file,array($currentMatchTo,''));
                    $h2 = '<h2>' . $value . '</h2>';
                    $first = false;
                }
                ...
                if(!is_array($value))
                    ...
                    foreach($value as $key =>$val) 
                    {
                        ...
                        $header .= "<td><strong>" . $key . "</strong></td>";
                        $buyerMatches=0;                        
                        if(array_key_exists($key,$masterArray[$i])){                            
                            ...
                            $str .="<td>" . number_format($percentMatch,2) . "%</td></tr>";
                            
                            fputcsv($file,array($curSup,number_format($percentMatch,2)));
                        }
                       else{
